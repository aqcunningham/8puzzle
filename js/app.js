...
SlidingBlock.prototype.copy = function(other){
    for (i = 0; i < 3; i += 1) 
        for (j = 0; j < 3; j += 1)
            this.disc[i][j] = other.disc[i][j];
    for (i = 0; i < other.moves.length; i += 1) 
...
SlidingBlock.prototype.handleInput = function(direction) {
    if (direction){
        var x0 = this.esx, x = x0, // old and new coords of empty slot 
            y0 = this.esy, y = y0;
        if (direction == 'left' && x0 > 0) 
            x -= 1;
        if (direction == 'right' && x0 < 2) 
            x += 1;
        if (direction == 'up' && y0 > 0) 
            y -= 1;
        if (direction == 'down' && y0 < 2) 
            y += 1;
        this.disc[x0][y0] = this.disc[x][y];
        this.disc[x][y] = 0;
        this.esx = x; this.esy = y;
        if (x0 != x || y0 != y) {
...
SlidingBlock.prototype.grid = function(){
    return ''+this.disc[0][0]+this.disc[0][1]+this.disc[0][2]
        +this.disc[1][0]+this.disc[1][1]+this.disc[1][2]
        +this.disc[2][0]+this.disc[2][1]+this.disc[2][2];
}

SlidingBlock.prototype.log = function(){
    log.innerHTML = '<b>Log:</b><br/>' + this.moves;
    log2.innerHTML // += '<br />'+this.moves;
        = '<b>Status:</b> </br><i>id</i>: ' + this.id
        + '; <i>empty slot</i> : (' + this.esx + ', ' + this.esy + '); ' 
        + '<i>allowed moves</i>: ' + this.getMove() + '</br>'
        + this.disc[0]+'</br>'+this.disc[1]+'</br>'+this.disc[2] + '</br><i>Cost</i>: ' + Math.round(this.cost * 100) / 100;
}
    ...
    if (shuffle) {
        for (i = 0; i < 3; i += 1) {
            for (j = 0; j < 3; j += 1) {
                ...
                this.disc[ii][jj] = tmp;
                if (this.disc[i][j] == 0) {this.esx = i; this.esy = j;}
            }
        }
    }
    this.render();
...
function solve() {
    ...
    node.copy(block);
    // we are ready f=to explore all neibours of node, so it allmost explored
    explored.add(node.grid());
    console.log(explored);
    while ( ! node.solved() && !queue.isEmpty() && cnt < 362880) { // 9! steps in worst case
        cnt += 1;
        // pop sli
        node = queue.pop();
        console.log('cnt = ' + cnt + '; heap size = ' + queue.size() + (node.solved()?'; !!!OK!!!':'; ---not ok---'));
        console.log(node);
